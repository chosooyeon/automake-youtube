...
const SEEDS: Seed[] = [
  ...
  ["career", 1, "복직 부서에 그룹 내 경력공모·사내 전배 제도 문의",
    "KB헬스케어는 KB금융그룹 계열사다. 외부에서 KB손보에 지원하는 사람과 그룹 계열사 7년차는 출발선이 다르고, 내부 이동은 코테를 안 보거나 약하게 보는 경우가 많다. 회사가 수익이 없고 물갈이가 이미 있었다면 더 급하다 — 받아줄 자리가 그룹 안에 있는지 없는지에 따라 나머지 11개월이 통째로 달라진다. 메일 한 통이면 끝난다.",
    "그룹 안에 자리가 있는지 없는지 판정 — 나머지 계획의 전제"],
  ["career", 1, "복직 예정일 확인 → 거기서 역산한 캘린더",
    "이 12주는 휴직 기간이 아니라 '복직까지 남은 시간'에 걸려 있다. 복직하면 공부 시간이 급감하고, 복직자는 물갈이 명단에 오르기 쉬운 자리이기도 하다. 남은 주 수를 실제로 세어본다.",
    "막연한 1년 → 세어본 주 수"],
  ["career", 1, "채용공고 10개 수집 → 요구사항 표로 (지원 안 함)",
    "전통금융(KB손보·은행)과 핀테크(토스·카뱅·카카오페이·신생증권)를 5개씩. 읽기만 한다. 뭘 요구하는지 재려는 것이고, 블록 2에서 뭘 공부할지가 추측이 아니라 이 표에서 정해진다.",
