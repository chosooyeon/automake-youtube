...
export default function Dashboard() {
  ...
  async function runFull() {
    if (!slug) return;
    const r = await fetch(`/api/projects/${encodeURIComponent(slug)}/run-all`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ upTo: "05-visual" }),
    });
    const j = await r.json();
    if (!j.ok) push({ kind: "error", title: "풀 실행 실패", message: j.error });
  }
