...
| **독립 배포판 (아버지용 퀀트 데스크)** | `~/Documents/quant` — 주식 모듈만 떼어낸 별도 git 저장소 (Next 앱이 루트, 포트 3200, 더블클릭 `시작.command`). **판정 로직 원본은 여기(본체)** 이고 그쪽은 `npm run sync` (`scripts/sync-from-automake.sh`) 로 끌어간다 — `lib/stock`·API·화면·CLI 를 복사하며 경로를 재작성. 그쪽 `config/*.json` 은 안 덮어쓴다 (아버지 관심종목). 주식 코드를 고쳤으면 그쪽에서 sync 한 번 |
| 주식 매매신호·알림 | `admin/lib/stock/*` (naver=데이터·indicators=지표·signals=판정·scan=알림). 관심종목은 `config/stock-watchlist.json` (커밋됨), 봇 토큰만 `admin/data/stock/telegram.json` (git 제외). 알림 이력 `config/stock-alert-state.json` 은 **CI 에선 Actions 캐시에 살고** 저장소의 사본은 캐시가 빌 때의 씨앗일 뿐 — 로컬 스캔이 건드려 git status 에 떠도 커밋 안 해도 된다. ⚠ CI 는 `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` 가 GitHub Secrets 에 있어야 돈다 (2026-08-13 부터 전 실행 실패 — 09-16 확인 시에도 시크릿 미등록 상태. 시크릿 확인 스텝에서 바로 죽으므로 알림이 한 통도 안 나간다) |
...
| 공부 노트 (클라우드·파이썬·라이브코테 — 취미·이직 준비, 콘텐츠 트랙 아님) | `admin/lib/study.ts`(타입·라이트너 복습) + `studyStore.ts`(파일 IO) + `components/StudyBoard.tsx` (탭 [📚 공부 노트]). **책 본문은 `config/study/{cloud,python,codingtest}.json`** (커밋됨 — 과목 1개 = 파일 1개, 파일만 추가하면 과목이 늘어남). 블록 6종 `table·code·note·flow·compare·tree`, 챕터마다 `cards`(플래시카드). 복습 상태는 `config/study-progress.json` (라이트너 5칸 1→3→7→14→30일). ⚠ **달성률·연속일수·경고 배너를 넣지 말 것** — 성적표가 되면 안 열게 된다 (퀘스트 탭과 같은 이유). 표는 [🙈 가리기]로 정의 열을 흐려 외운다 |
